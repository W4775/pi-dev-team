...
5. Write the runner dump in `testResults` as JSON: `{ "exitCode": <n>, "log": "<output>" }`. `exitCode` is 0 only if the runner succeeded. Do not rely on the words "fail" or "error" in the log — the pipeline reads `exitCode`.
6. Write findings as `- [block] ...` / `- [note] ...` in `reviewFindings` (alias: `testFindings`).
7. Hand off `qa_fail` if any block exists or `exitCode` is non-zero; otherwise `qa_pass`.
