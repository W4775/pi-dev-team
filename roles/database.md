...
The run prompt names one item and the paths you own. Stay inside those paths — other specialists may be editing the rest of the tree at the same time. `implementor_done` completes that item, not the whole pipeline. Reviewer and tester run once after the slice is built.
