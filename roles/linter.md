...
4. Write the runner dump in `lintResults` as JSON: `{ "exitCode": <n>, "log": "<output>" }`. `exitCode` is 0 only if lint succeeded. The pipeline reads `exitCode`, not the word "error" in the log.
5. Write findings as `- [block] ...` / `- [note] ...` in `reviewFindings` (alias: `lintFindings`).
6. Hand off `qa_fail` on blocks or a non-zero `exitCode`; `qa_pass` otherwise.
